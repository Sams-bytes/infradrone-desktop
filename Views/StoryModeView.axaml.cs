    ...
    public partial class StoryModeView : UserControl
    {
        ...
        private readonly (string Path, string Narrative)[] _steps = new[]
        {
            ...
            (Base + "analytics_near_miss_hotspot.png",
             "STEP 4 -- ALL 19 NEAR-MISSES, MAPPED.  Each red circle is one automatically-detected near-miss (19 distinct pedestrian-vehicle event pairs, drawn from 337 individual flagged moments across the recording); bigger circle = more severe (lower reaction time). This is Time-To-Collision analysis: real math, run on real trajectories, with zero human watching the footage to spot it."),

            (Base + "trajectory_map.png",
             "STEP 5 -- ZOOMING INTO ONE CASE.  We're now following the single most severe event: pedestrian 910 (red path) and vehicle 942 (teal path), labeled directly on the map. Minimum distance between them: 4.9 meters. Time-To-Collision: 1.19 seconds -- just over one second of reaction time. This is a textbook-severe near-miss by published traffic-safety standards."),
